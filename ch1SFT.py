def ch1(fs, dt, N, fmin, fmax,tmax ,tmin ,wf , vm, dBvalve, plotres, TimeFrequency, dataharvest, complexdiag, Timedomain,
    ...
    for info, name in zip(filelist, filename):
        ...
        file = os.path.join(domain,info)                                                                                    #將文件夾路徑及檔案名稱拼湊為完整路徑
        if filetype=='csv':
    ...
    #####利用短時傅立葉進行信號處理，並將計算結果以熱點圖呈現
                ...
                if check>dBvalve:
                    ...
                    if TimeFrequency == 1:
                        if skip==0:
                            ...
                            plt.colorbar()
                            plt.savefig(plotpath+'/'+str(name1)+'_STFT',dpi=600, bbox_inches="tight")
                            plt.close('all')
        ...
        if dataharvest==1:
            if overdBvalve==1:
                ...
                SPLdB = SPLdB.T
                np.savetxt(plotpath + '/' + str(name)+'mic', SPLdB, fmt='%f', delimiter='\t',
        ...
        if complexdiag == 1:
            if len(labelt):
                ...
                plt.savefig(plotpath + '/' + str(name)+'_ID', dpi=600, bbox_inches="tight")
                plt.close('all')
